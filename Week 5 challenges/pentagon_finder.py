...
# Load template pentagon
pentagon = cv2.imread('..\Photos\pentagon.png')
pentagonCanny = cv2.Canny(pentagon,100,200) #make a canny
pentagonContours, hierarchy = cv2.findContours(pentagonCanny,cv2.RETR_LIST,cv2.CHAIN_APPROX_NONE) #find the contours of the pentagon
...
cv2.imshow("heart", pentagonBlank)
cv2.imshow("heartContour", pentagonCanny)
